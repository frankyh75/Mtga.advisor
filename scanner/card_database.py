...
ENRICHED_CACHE_SCHEMA = "enriched-cards.v1"
...
def _enriched_lookup_file() -> Path:
    return get_default_cache_dir() / "enriched_card_db.json"
...
# ---------------------------------------------------------------------------
# Enriched card database — full Scryfall metadata for the collection browser.
# Stores rarity, cmc, type_line, colors, oracle_text, image_uri per Arena ID.
# ---------------------------------------------------------------------------

# Maps Scryfall rarity codes to MTGA rarity names.
_SCRYFALL_RARITY_MAP = {
    "common": "common",
    "uncommon": "uncommon",
    "rare": "rare",
    "mythic": "mythic",
...
def _add_enriched_scryfall_card(lookup: dict[int, dict[str, Any]], card: dict[str, Any]) -> None:
    """Add a Scryfall card with full metadata if it has an Arena ID."""
    arena_id = card.get("arena_id")
    if not arena_id:
        return

    colors = card.get("colors", [])
    # For double-faced cards, merge colors from all faces.
    ...
    rarity = _SCRYFALL_RARITY_MAP.get(card.get("rarity", ""), card.get("rarity", ""))
...
def fetch_enriched_scryfall_database() -> dict[int, dict[str, Any]]:
    """Fetch the Scryfall bulk-data default-cards dump and extract enriched metadata.

    Returns a dict mapping Arena ID → card metadata with rarity, cmc, type_line,
    colors, oracle_text, image_uri.
    """
    try:
        import requests
    except ImportError:
        print("❌ requests ist nicht installiert. Enriched Scryfall-Fallback nicht verfügbar.")
        return {}

    print("🌐 Lade enriched Kartendaten von Scryfall API...")
    try:
        headers = {
            "Accept": "application/json",
            "User-Agent": "Mtga.advisor/0.1",
        ...
        if str(download_uri).endswith(".jsonl.gz"):
            with gzip.GzipFile(fileobj=cards_response.raw) as gz:
                for raw_line in gz:
                    if not raw_line.strip():
                        continue
                    _add_enriched_scryfall_card(lookup, json.loads(raw_line))
            return lookup

        for c in cards_response.json():
            _add_enriched_scryfall_card(lookup, c)
        return lookup
    except Exception as e:
        print(f"❌ Enriched Scryfall-Download fehlgeschlagen: {e}")
        return {}


def _read_enriched_cache(path: Path) -> dict[int, dict[str, Any]] | None:
    """Read the enriched cache file. Returns None if missing/corrupt."""
    try:
        with path.open("r", encoding="utf-8") as f:
            data = json.load(f)
        if isinstance(data, dict) and data.get("schema") == ENRICHED_CACHE_SCHEMA:
            cards = data.get("cards", {})
            return {int(k): v for k, v in cards.items() if isinstance(v, dict)}
    except Exception:
        pass
    return None


def _write_enriched_cache(path: Path, lookup: dict[int, dict[str, Any]]) -> None:
    payload = {
        "schema": ENRICHED_CACHE_SCHEMA,
        "cardCount": len(lookup),
        "cards": {str(k): v for k, v in sorted(lookup.items())},
    }
    with path.open("w", encoding="utf-8") as f:
...
def load_enriched_card_database(*, refresh_cache: bool = False) -> dict[int, dict[str, Any]]:
    """Load enriched card metadata (rarity, cmc, type_line, colors, image_uri, oracle_text).

    Reihenfolge:
      1. Cache-Datei (~/.mtga_advisor/enriched_card_db.json)
      2. Scryfall Bulk-API (Fallback)

    Falls Scryfall nicht verfügbar ist, wird ein leeres Dict zurückgegeben.
    Der Caller (Endpoint) sollte dann nur die Basis-Info aus dem normalen
    Lookup liefern.
    """
    cache_dir = get_default_cache_dir()
    cache_dir.mkdir(parents=True, exist_ok=True)

    enriched_file = _enriched_lookup_file()

    # 1. Cache
    if enriched_file.exists() and not refresh_cache:
        cached = _read_enriched_cache(enriched_file)
        if cached is not None:
            print(f"📦 Enriched Karten-DB aus Cache geladen ({len(cached)} Karten)")
            return cached

    # 2. Scryfall
    print("🔄 Lade enriched Karten-DB von Scryfall...")
    lookup = fetch_enriched_scryfall_database()

    # Cache schreiben
    if lookup:
        try:
            _write_enriched_cache(enriched_file, lookup)
            print(f"💾 Enriched Karten-DB gecached ({len(lookup)} Karten)")
        except Exception as exc:
            print(f"⚠ Enriched Cache konnte nicht geschrieben werden: {exc}")
